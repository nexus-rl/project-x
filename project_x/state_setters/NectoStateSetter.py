...
class NectoStateSetter(StateSetter):
    def __init__(
        ...
        self.replay_setters = [
            ...
            for replay_array in read_replay_arrays()
        ]
        self.setters = [
            BetterRandom(),
            DefaultState(),
...
def read_replay_arrays():
    _replay_base_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "data","necto_state_setter"))
    _replay_paths = os.listdir(_replay_base_path)
    return [np.load(path) for path in _replay_paths if path.endswith(".npy")]
